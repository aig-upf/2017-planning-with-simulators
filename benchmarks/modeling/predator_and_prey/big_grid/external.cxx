    ...
    _preys(info.getTypeObjects("prey"))
{
    ...
    for ( auto o : _predators ) {
        _predator_location_vars[o] = info.resolveStateVariable( loc_sym_id, {o} );
        _x_predator[o] = 0;
        _y_predator[o] = 0;
    }
    for ( auto o : _preys ) {
...
External::next_move(const State& state, const std::vector<ObjectIdx>& args ) const {
    ...
    for ( auto o : _predators ) {
        ObjectIdx o_pos = state.getValue(_predator_location_vars.at(o));
...
External::trapped(const State& s, const std::vector<ObjectIdx>& args ) const {
    ...
    for ( auto o : _predators ) {
        ObjectIdx o_pos = s.getValue(_predator_location_vars.at(o));
