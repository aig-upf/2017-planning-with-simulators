...
public:
    ...
    //! Distance function
    ObjectIdx  distance(const State& state, const std::vector<ObjectIdx>& arguments ) const;
    ObjectIdx  distance(const std::vector<ObjectIdx>& arguments ) const;
...
protected:
    ...
    std::unordered_map<ObjectIdx,VariableIdx>                   _location_vars;
...
class DistanceTerm : public fs::AxiomaticTerm {
public:
    DistanceTerm( const std::vector<const fs::Term*>& subterms );

	DistanceTerm* clone(const std::vector<const fs::Term*>& subterms) const override;
